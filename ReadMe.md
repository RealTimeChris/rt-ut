...
It exists to run the test suites for the rest of the stack (void-numerics,
Jsonifier). Those libraries anchor their correctness on external, authoritative
conformance corpora — the LLVM libc++ `<charconv>` suite, the standard JSON
conformance and fail-case suites. rt-ut is the runner that executes them and
reports results; the authority lives in the test data, not in this framework.
...
```bash
cmake -S . -B build -DRT_UT_TESTING=ON
cmake --build build
./build/test/rt-ut-main        # .\build\test\Release\rt-ut-main.exe on Windows
```
